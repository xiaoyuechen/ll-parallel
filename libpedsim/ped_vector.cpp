...
std::string Ped::Tvector::to_string() const {
  ...
  strs << x << "/" << y << "/" << z;
  return strs.str();
  // return std::to_string((const long double) x) + "/" + std::to_string((const
  // long double) y_) + "/" + std::to_string((const long double) z_);
}
...
/// Returns the length of the vector squared. This is faster than the real
/// length. \return the length squared
...
/// given factor. \date    2013-07-16 \return  The scaled vector. \param factor
...
double Ped::Tvector::polarAngle() const { return atan2(y, x); }
