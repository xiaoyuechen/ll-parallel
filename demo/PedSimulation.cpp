...
int PedSimulation::getTickCount() const { return tickCounter; }
...
void PedSimulation::runSimulationWithQt(int maxNumberOfStepsToSimulate) {
  ...
  // movetimer.setInterval(50); // Limits the simulation to 20 FPS (if one so
  // whiches).
  QObject::connect(&movetimer, SIGNAL(timeout()), this,
                   SLOT(simulateOneStep()));
  movetimer.start();
}
